...
CREATE TABLE IF NOT EXISTS users (
 id uuid PRIMARY KEY, email text NOT NULL, deleted_at timestamptz, deletion_completed_at timestamptz,
 created_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS wallets (
 user_id uuid PRIMARY KEY REFERENCES users(id), balance integer NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS members (
 book_id uuid REFERENCES books(id) ON DELETE CASCADE, user_id uuid REFERENCES users(id),
...
CREATE TABLE IF NOT EXISTS invites (
 ...
 expires_at timestamptz NOT NULL, accepted_by uuid REFERENCES users(id), revoked boolean NOT NULL DEFAULT false
);
CREATE TABLE IF NOT EXISTS archives (
 id uuid PRIMARY KEY, user_id uuid REFERENCES users(id), book_id uuid REFERENCES books(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS purchases (
 ...
 user_id uuid REFERENCES users(id), product_id text NOT NULL, credits integer NOT NULL CHECK(credits>0),
...
CREATE TABLE IF NOT EXISTS ledger (
 id uuid PRIMARY KEY, user_id uuid REFERENCES users(id), delta integer NOT NULL,
