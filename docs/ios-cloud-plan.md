...
## 取り込み履歴からのスワイプ復帰（2026-09-13）

- 取り込み履歴では画面左端から右へのスワイプで取り込み画面へ戻す。既存の戻るボタンは維持し、縦スクロール・検索や状態絞り込みとの競合を避ける。
- 型検査と差分検査を行い、実機でのジェスチャー操作は別途確認する。

## Issue #33: カード化時の同意確認（2026-09-13）

- 取り込み画面の「カード化の確認」セクションと事前同意トグルを削除する。保存済みページと履歴からの再試行は、トグル操作なしで開始できる。
- カード化ボタンを押した時に見積もりを取得し、OpenAIへのHTML送信、最大使用権数、差分返却・失敗時の扱いを確認ダイアログに表示する。利用者が「カード化する」を選んだ場合に限り `consent: true` で送信する。
- モバイル型検査と実画面で確認し、実機配布版の確認は別途記録する。

## Issue #30: 献立とレシピカードの往復（2026-09-13）

- レシピ一覧・詳細から当日の献立へ直接追加する。保存済みの献立を読み直して同じレシピの重複を防ぎ、追加後はサーバーへ保存する。閲覧専用の参加者には追加操作を表示しない。
...
カード化したレシピは物理削除せず、owner/editorがアーカイブ・復元できる。通常のレシピ一覧と新しい献立の追加候補からは除外し、アーカイブ済み一覧で閲覧・復元できる。過去の献立に登録済みのカードは、アーカイブ後も献立から詳細を開ける。APIで新しい献立にアーカイブ済みカードを追加することも拒否する。
