...
## Issue #51: 下部ナビゲーションとセーフエリア（2026-09-14）

- ホーム画面では下端のセーフエリアをタブバー自身が取り込み、背景をホームインジケータまで連続させる。タブの操作領域と選択状態は維持する。
- ログインなどタブバーのない画面は従来どおり下端のセーフエリアを確保する。型検査と実画面を確認する。

## Issue #50: レシピカードの上部操作（2026-09-14）

- 献立とレシピ一覧から開く page sheet で、上部の戻るボタンを表示せず、カードを上から表示する。シートの下スワイプによる復帰を維持する。
- モバイル型検査と差分を確認し、両方のシートの表示・下スワイプ操作は Simulator で確認する。

## Issue #45: 取り込み履歴の画面構造と余白（2026-09-14）

- 履歴を取り込みタブの内容差し替えから独立した iOS page sheet に変更する。閉じる操作と下スワイプで戻り、元画面の入力状態を保持する。
- 検索・絞り込み・一覧・追加読み込みを共通余白と要素間隔を持つ一つのスクロール領域に置く。データ取得、権限、再試行、原本共有は維持する。
- 型検査と画面表示を確認する。実機配布版は別途検証する。

## Issue #47: アーカイブ済み画面の作成セクション（2026-09-14）

- 設定のアーカイブ済みレシピ画面では「レシピ帖を作る」を表示しない。通常の設定画面とレシピ帖未作成時の献立画面では、従来どおり表示する。

## Issue #44: レシピ一覧の更新操作（2026-09-14）

- レシピ一覧上部の「最新のレシピを読む」ボタンを削除する。
- 画面の引き下げ更新で、レシピ帖・残高に加えて表示中の通常／アーカイブ済みレシピ一覧も再取得する。APIと保存データは変更しない。
- モバイル型検査と差分を確認し、実機の引き下げ操作は別途検証する。

## Issue #41: レシピ一覧の献立追加ボタン（2026-09-14）

- レシピ一覧では、各カードの詳細表示領域と「今日の献立に追加」操作を同じカード内に収め、区切り線で操作を分ける。追加済み・編集中・閲覧専用・アーカイブ済みの既存動作を維持する。
- モバイル型検査とカード表示を確認する。実機配布版での確認は別途記録する。

判定方針更新: 2026-09-12

## Issue #39: アーカイブ済みレシピの導線（2026-09-14）

- 通常のレシピ画面は未アーカイブだけを表示し、上部の切り替えボタンをなくす。
- 設定からレシピ帖ごとのアーカイブ済み一覧へ進める。編集権限がある参加者は一覧から直接アーカイブを解除できる。閲覧者は閲覧のみ。
- 既存のアーカイブAPIを利用し、APIや保存データは変更しない。
...
Simulatorの開発用ポート（4330など）は常に実抽出モードとし、固定HTML・固定カードを返さない。公開URLの安全な取得とサーバー側OpenAI抽出を本番と同じ実装で行い、Clerk development認証とOpenAI設定がなければ起動を拒否する。データと取り込み権は従来どおりプロセス内のテスト用に限定し、S3・RevenueCat・本番DBへは接続しない。旧TestFlight向けTailnet fixtureの4329番だけは固定UI用として残し、実抽出を拒否する。モデル呼び出しは取り込み1件あたり最大30回とし、別の取り込みへ上限を持ち越さない。外部のOpenAI料金が発生し、Simulator実抽出検証と本番サービス受入は分けて記録する。
