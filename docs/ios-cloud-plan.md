...
Simulatorの開発用ポート（4330など）は常に実抽出モードとし、固定HTML・固定カードを返さない。公開URLの安全な取得とサーバー側OpenAI抽出を本番と同じ実装で行い、Clerk development認証とOpenAI設定がなければ起動を拒否する。データと取り込み権は従来どおりプロセス内のテスト用に限定し、S3・RevenueCat・本番DBへは接続しない。旧TestFlight向けTailnet fixtureの4329番だけは固定UI用として残し、実抽出を拒否する。モデル呼び出し回数を制限する。外部のOpenAI料金が発生し、Simulator実抽出検証と本番サービス受入は分けて記録する。
