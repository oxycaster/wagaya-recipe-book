...
- prodのClerk、実S3、公開API/worker、実OpenAIは未接続。Crunchy BridgeはHobby plan・HAなし・log drain未設定で、別クラスタへのバックアップ復元も未検証。
...
| 取り込み権 | Consumable 10回分・50回分 | 個数のみ実装済み、価格未決定 |
...
- 10回分・50回分の税込価格、商品ID、返金問い合わせ、障害時の権利返還方針を確定する。
...
- 代表的な10〜20サイトと、複数レシピ、ログイン必須、巨大HTML、画像なし、拒否、タイムアウトを受入標本にする。原本は保存し、複数候補を自動確定しない。
