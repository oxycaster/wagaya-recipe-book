...
3. dev Docker PostgreSQL、prod Crunchy Bridge PostgreSQLは受入済み。次はAWS管理者がproduction/develop profilesへCDK bootstrap/deployに必要な権限を付与し、`infra/cdk/README.md` の初回作成を実行する。prod stack出力のGitHub roleをrepository environmentへ設定し、運用設定secretを入力する。CDK diffとLightsail 2GBの実アカウント上の月額を確認後にprod stackを作成し、DBの暫定operator IPをstatic IPへ置換する。Production Checkで残るproduction instance/HA/log drain、別クラスタへのバックアップ復元はG2完了前に判断・検証する。外部環境ではA/B/Cユーザーのアクセス境界、実HTMLの保存と抽出、Sandbox購入の重複/返金/復元、アカウント削除のS3/Auth消去を確認する。
