...
3. dev Docker PostgreSQL、prod Crunchy Bridge PostgreSQL、dev/prod CDK stack、親DNSは作成済み。production `application settings` secretにはDB/Clerkだけを入力済み。次はOpenAI API key/modelとRevenueCatのWebhook secret、App ID、商品対応表を作成して入力し、GitHub ActionsのOIDC更新を確認する。初回runtime deploy後にCrunchy Bridgeの暫定operator IPをLightsail static IPへ置換し、HTTPS/Clerk/S3/LLMの実受入を行う。Production Checkで残るproduction instance/HA/log drain、別クラスタへのバックアップ復元はG2完了前に判断・検証する。外部環境ではA/B/Cユーザーのアクセス境界、実HTMLの保存と抽出、Sandbox購入の重複/返金/復元、アカウント削除のS3/Auth消去を確認する。
