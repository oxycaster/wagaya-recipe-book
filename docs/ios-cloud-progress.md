...
3. dev Docker PostgreSQL、prod Crunchy Bridge PostgreSQL、dev/prod CDK stack、親DNSは作成済み。production `application settings` secretにはDB/Clerk/OpenAIとRevenueCatの商品対応表を含むruntime設定を入力済み。RevenueCat WebhookとApp Store Connect API連携も保存済み。現ブランチをmainへ反映し、GitHub ActionsのOIDC deployを確認する。初回runtime deploy後にCrunchy Bridgeの暫定operator IPをLightsail static IPへ置換し、HTTPS/Clerk/S3/LLMの実受入を行う。Production Checkで残るproduction instance/HA/log drain、別クラスタへのバックアップ復元はG2完了前に判断・検証する。外部環境ではA/B/Cユーザーのアクセス境界、実HTMLの保存と抽出、20〜50件のusage/P95原価、Sandbox購入の重複/返金/復元、アカウント削除のS3/Auth消去を確認する。App Store Connectの審査用メタデータを補完し、RevenueCatの `Missing Metadata` を解消する。
